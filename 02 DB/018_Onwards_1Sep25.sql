...
ALTER PROCEDURE [Onwards].[GetAttendanceRegularizationForManager]
	@ManagerId INT 
AS
BEGIN
	...
	SELECT AR.Id,AR.UserId, U.FullName, AR.StartDate, AR.EndDate, AR.Duration,AR.Reason,AR.Action, LS.Name AS StatusName
	FROM Onwards.AttendanceRegularization AS AR
	INNER JOIN Onwards.Users AS U ON U.Id = AR.UserId
	INNER JOIN Onwards.LeaveStatus AS LS ON AR.StatusId = LS.Id
	WHERE AR.ManagerId = @ManagerId AND AR.StatusId IN (1,2) -- 1=> Pending,2=> Approved
	ORDER BY AR.CreatedDate DESC

END

ALTER PROCEDURE [Onwards].[GetUserLeaveAppliedForManager]
	@ManagerId INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT 
		   ...
		   ULA.Reason,
		   ULA.Action,
		   Un.FullName AS NotifiedUserName,
		   LT.LeaveTypeName,
		   LS.Name AS LeaveStatusName,
		   ULA.FileName
	FROM Onwards.UserLeaveApplied AS ULA
	INNER JOIN Onwards.Users AS U ON U.Id = ULA.UserId
	INNER JOIN Onwards.Users AS Un ON Un.Id = ULA.NotifiedUserId
...
ALTER PROCEDURE [Onwards].[UpdateUserLeaveApplied]
    @Request Onwards.UpdateULATVP READONLY
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        -- Cursor-like iteration through TVP rows
        DECLARE cur CURSOR LOCAL FAST_FORWARD FOR
        SELECT Id, StartDate, EndDate, NoOfDays, Action, LeaveTypeId, LeaveStatusId, UserId, LoginId
        FROM @Request;
        ...
        FETCH NEXT FROM cur INTO 
            @Id, @StartDate, @EndDate, @NoOfDays, @Action, @LeaveTypeId, @LeaveStatusId, @UserId, @LoginId;

        WHILE @@FETCH_STATUS = 0
        BEGIN
            DECLARE @Dates TABLE (Date DATE);

            -- 🟡 If Rejected (3) or Cancelled (4)
            IF (@LeaveStatusId IN (3, 4))
            BEGIN
                -- ✅ 1️⃣ Restore Leave Balance
                UPDATE Onwards.LeaveBalances
                SET RemainingDays = RemainingDays + @NoOfDays
                WHERE UserId = @UserId AND LeaveTypeId = @LeaveTypeId;

                -- ✅ 2️⃣ Handle case where leave period is completely past
                ...
                -- ✅ 3️⃣ Handle current-day overlap case
            ...
            -- ✅ 4️⃣ Update the main UserLeaveApplied record
...
CREATE TYPE Onwards.UpdateARTVP AS TABLE 
(
	Id INT,
	StartDate DATE NULL,
	EndDate DATE NULL,
...
ALTER PROCEDURE Onwards.UpdateAttendanceRegularization
    @Request Onwards.UpdateARTVP READONLY
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        -- 1️⃣ Update AttendanceRegularization
        MERGE Onwards.AttendanceRegularization AS Target
        ...
        -- 2️⃣ Update UserShiftLog (single WHEN MATCHED using CASE)
        MERGE Onwards.UserShiftLog AS Target
        USING @Request AS Source
        ON Target.UserId = Source.UserId
           AND Target.Date BETWEEN Source.StartDate AND Source.EndDate
        WHEN MATCHED THEN
            UPDATE 
            SET 
                Target.IsPresent = CASE 
                                      WHEN Source.StatusId = 2 THEN 1   -- Approved
                                      WHEN Source.StatusId = 3 THEN 0   -- Rejected
                                      ELSE Target.IsPresent              -- unchanged
                                   END,
                Target.ModifiedDate = GETDATE(),
                Target.ModifiedBy = Source.LoginId;
        ;
...
INSERT INTO Onwards.AttendanceRegularizationType (Id,Type)
VALUES
(1,'Day'),
(2,'Time')


CREATE TABLE Onwards.AttendanceRegularization
(
	...
	CONSTRAINT FK_AttendanceRegularization_Users FOREIGN KEY(UserId) REFERENCES Onwards.Users(Id),
	CONSTRAINT FK_AttendanceRegularization_AttendanceRegularizationType FOREIGN KEY(TypeId) REFERENCES Onwards.AttendanceRegularizationType(Id),
	CONSTRAINT FK_AttendanceRegularization_LeaveStatus FOREIGN KEY(StatusId) REFERENCES Onwards.LeaveStatus(Id),
)

ALTER PROCEDURE Onwards.InsertAttendanceRegularization
	@UserId INT ,
	@TypeId INT ,
	@StartDate DATE ,
	@EndDate DATE ,
...
BEGIN
	...
	BEGIN
		RAISERROR('Cant be more than 30 days from today', 16, 1);
        RETURN;
	END

	DECLARE @ManagerId INT ;
	SELECT @ManagerId = ReportingManagerId 
	FROM Onwards.Users
	WHERE Id = @UserId

	INSERT INTO Onwards.AttendanceRegularization
			(UserId ,
			ManagerId,
			TypeId ,
			StartDate,
...
CREATE PROCEDURE Onwards.GetAttendanceRegularization
	@UserId INT 
AS
BEGIN
...
EXEC sp_add_job 
    @job_name = N'Daily Midnight Service';

EXEC sp_add_jobstep 
    @job_name = N'Daily Midnight Service',
    @step_name = N'Perform Start-of-Day Tasks',
    @subsystem = N'TSQL',
...
-- 3️ Define a Schedule (Daily at midnight)
...
-- 4️ Attach Schedule to Job
EXEC sp_attach_schedule 
    @job_name = N'Daily Midnight Service',
    @schedule_name = N'Daily at Midnight';

-- 5️ Add the Job to SQL Server Agent
EXEC sp_add_jobserver 
    @job_name = N'Daily Midnight Service';
GO


CREATE TABLE Onwards.UserType
(
	Id INT PRIMARY KEY,
...
INSERT INTO Onwards.UserType (Id, TypeName)
VALUES 
    (1, 'Permanent'),
    (2, 'Contract'),
    (3, 'Probation Period');
...
ADD UserTypeId INT NOT NULL DEFAULT 1

ALTER TABLE Onwards.Users 
ADD CONSTRAINT FK_Users_UserType FOREIGN KEY (UserTypeId) REFERENCES Onwards.UserType(Id)
...
BEGIN
	...
	-- Week off check
	IF (DATENAME(WEEKDAY, @Yesterday) IN ('Saturday', 'Sunday'))
	BEGIN
		Return;
	END

	-- Active Users who does'nt have a holiday on yesterday
	INSERT INTO @UserDetails (UserId,ShiftId)
	SELECT U.Id,S.ShiftId
	FROM Onwards.Users AS U
	INNER JOIN Onwards.UserShiftAssignment AS S ON U.Id = S.UserId
	WHERE U.IsActive = 1 AND S.IsActive = 1 AND
	NOT EXISTS(SELECT 1 FROM Onwards.HolidayList WHERE HolidayDate = @Yesterday AND LocationId = U.LocationId)

	-- Adding absent for users who have'nt logged in yesterday
	MERGE Onwards.UserShiftLog AS T
	USING @UserDetails AS S
...
CREATE PROCEDURE Onwards.ApproveLeaves
AS 
BEGIN
	SET NOCOUNT ON;
	...
	MERGE Onwards.UserLeaveApplied AS T
	USING Onwards.Users AS S
	ON T.UserId = S.Id AND
	CAST(T.CreatedDate AS DATE) = @Yesterday AND
	-- 1 --> Requested
	T.LeaveStatusId = 1 AND
	S.IsActive = 1
	WHEN MATCHED THEN
		UPDATE 
		-- 2 --> Approved
		SET T.LeaveStatusId = 2;
END
...
BEGIN
	...
	BEGIN TRANSACTION;
		...
		SELECT U.Id , L.LeaveTypeId,L.NoOfLeaves , @Today, 0
		FROM Onwards.Users AS U
		-- 1 --> Sick Leave , 2 --> Casual Leave
		CROSS JOIN (VALUES (1, 1), (2, 1)) AS L(LeaveTypeId, NoOfLeaves)
		WHERE DATEDIFF ( DAY , U.DOJ , @Today ) > 15 AND U.IsActive = 1
		...
		FROM Onwards.Users AS U
		-- 1 --> Sick Leave , 2 --> Casual Leave
		CROSS JOIN (VALUES (1, 0.5), (2, 0.5)) AS L(LeaveTypeId, NoOfLeaves)
		WHERE DATEDIFF ( DAY , U.DOJ , @Today ) <= 15 AND U.IsActive = 1
		
		MERGE Onwards.LeaveBalances AS T
		USING Onwards.LeavesAdded AS S
		ON T.UserId = S.UserId AND T.LeaveTypeId = S.LeaveTypeId AND CAST(S.CreatedDate AS DATE) = @Today 
		WHEN MATCHED THEN
			UPDATE 
			SET T.RemainingDays = T.RemainingDays + S.NoOfLeaves;
...
BEGIN
	...
	BEGIN TRANSACTION;
		...
		DECLARE @WorkingDays DECIMAL(9,2);
...
BEGIN
	...
	WHERE LT.IsActive = 1 AND LB.UserId = @UserId
END
...
ALTER PROCEDURE Onwards.GetCalendarEvents
	@UserId INT,
	@Month INT,
	@Year INT
AS 
BEGIN
	...
	BEGIN TRANSACTION;
		...
		--  2 --> Approved
		(LeaveStatusId = 2) AND
		((CAST(StartDate AS DATE) BETWEEN @StartDate AND @EndDate) OR (CAST(EndDate AS DATE) BETWEEN @StartDate AND @EndDate)) AND
		(NoOfDays != 0.5)
		...
		BEGIN
			...
			-- Holiday Check
			IF EXISTS( SELECT 1 FROM Onwards.HolidayList WHERE HolidayDate = @DateItr AND LocationId = @LocationId)
			BEGIN
				INSERT INTO @CalenderDates (Date, StatusId)
				-- 5 --> Holiday
				VALUES (@DateItr, 5); 
		...
		MERGE @CalenderDates AS T
		USING Onwards.UserShiftLog AS S
		ON S.UserId = @UserId AND T.Date = S.Date AND T.StatusId = 1 -- 1 => Present 
		WHEN MATCHED THEN
			UPDATE 
			SET 
